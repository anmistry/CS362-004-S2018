...
// five refactored playing cards are below with bugs introduced
int smithyCard (int currentPlayer, struct gameState *state, int handPos) {
    //+3 Cards
    int i;
    
    for (i = 0; i < 4; i++) //changed to four instead of 3
    {
        printf("drawCard\n");
...
int remodelCard(int currentPlayer, struct gameState *state, int handPos, int choice1, int choice2) {
    ...
    return -1; //returns -1 instead of 0
...
int salvagerCard(int currentPlayer, struct gameState *state, int handPos, int choice1) {
    ...
    if (choice1)
    {
        ...
        //trash card
        discardCard(choice1, currentPlayer, state, 0);//changed trash card state to 0 instead of 1
    }
    
    //discard card
    discardCard(handPos, currentPlayer, state, 1); //changed trash card state to 1 instead of 0
    return 0;
}
...
return 0; 
   case council_room:
    ...
    case remodel: //use refactored function
    ...
    case tribute: //use refactored function
            tributeCard(currentPlayer, nextPlayer, state); //a tribute to the greatest card in the world
    ...
    case salvager: //use refactored card
